...
- Mapping: MapStruct
  ...
  1. jspecify를 사용해 package-info.java를 등록하고, nullMark로 안전하게 프로젝트를 진행할 수 있도록 했음
  2. 엔티티 간 연관관계 처리 방식으로 엔티티는 연관관계를 맺는 대신 ID만 참조하기로 결정
  3. ER다이어그램
  <img width="1031" height="581" alt="image" src="https://github.com/user-attachments/assets/a3fa67bc-c993-47e9-8d4c-f077cd3ef08f" />


---
## 깃허브 중심 개발 진행
- issue 발행 및 discussion 기능으로 문제 정의 및 소통으로 결과를 도출
- PR시 팀원간 코드리뷰 진행 및 피드백

---
## 개발 컨벤션
- Commit 컨벤션
  - FEAT(기능 추가), FIX(수정), CHORE(기타 기능),DELETE, REFACTOR, TEST
- 코드 스타일
  - Intelli J 코드 스타일
- 개발 원칙
  - 엔지니어 3원칙(KISS, YAGNI, DRY) 염두
  
---
## 아키텍처 설계
- 패키지 구조
  - 기능 별 패키지 분리가 아닌 도메인 별 패키지 분리
 
- ServiceLayer 설계
  - (입력/출력) Service는 Controller에 DTO를 반환하며, Mapper의 위치도 Service Layer로 결정
- 데이터 조회 방식
  - JPQL, Projection, QueryDSL 등은 특정 기술을 강제하지 않고, 상황에 맞게 선택적으로 적용
---
...
기존에 깃을 사용할떄마다 헷갈렸었으며, 깃 명령어에도 익숙치 않았습니다.

원본 레포지토리의 최신 반영사항을 얻어오기위해 upstream도 처음 사용해보았습니다. 

이번 프로젝트를 마무리하면서, git 사용하는데 있어 처음보다 덜 두려워진것 같습니다.

또한 프로젝트 진행중 팀원들이 제가 `모릅니다`한것들을 바로 알려주셔서 많은것을 배웠습니다.
